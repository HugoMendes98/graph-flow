...
const authConnectAs = (email: string, password: string) =>
	cy.request(
		"post",
		`${e2eApi}${AUTH_ENDPOINT_PREFIX}/${AuthEndpoints.LOGIN}`,
		{
			cookie: true,
			email,
			password
...
const authDisconnect = () =>
	cy
		.request(
			"post",
			`${e2eApi}${AUTH_ENDPOINT_PREFIX}/${AuthEndpoints.LOGOUT}`
		)
		.then(() => cy.clearCookies());

const dbRefresh = (sample: DbTestSample) =>
	cy.wrap(DbE2eHelper.getHelper(sample).refresh());
