...
import {
	ApiGraphParam,
	GraphInterceptedParam,
	GraphInterceptor
} from "../graph.interceptor";
...
type EndpointTransformed = {
	...
	[K in keyof EndpointBase]: UnshiftParameters<
		EndpointBase[K],
		[GraphEntity]
	>;
};
...
@Controller(
	generateGraphArcsEndpoint(
		`:${GraphInterceptor.PATH_PARAM}` as unknown as EntityId
	)
)
@UseAuth()
...
export class GraphArcController implements EndpointTransformed {
	...
	@Post()
	public create(
		@GraphInterceptedParam() _graph: GraphEntity,
		@Body() body: GraphArcCreateDto
	) {
		// TODO: verify graph
	...
	@Patch("/:id")
	public update(
		@GraphInterceptedParam() _: GraphEntity,
		@Param("id") id: number
	) {
	...
	public delete(
		...
		return this.validateArcId(graph, id).then(({ _id }) =>
	 ...
	 */
	private validateArcId(
		graph: GraphEntity,
		id: number
	): Promise<GraphArcEntity> {
		return this.service
			.findById(id, {
				populate: { from: { node: true }, to: { node: true } }
			})
			.then(arc => {
				for (const kind of [arc.from.node.kind, arc.to.node.kind]) {
					if (
						kind.type !== NodeKindType.VERTEX ||
						kind.__graph !== graph._id
					) {
