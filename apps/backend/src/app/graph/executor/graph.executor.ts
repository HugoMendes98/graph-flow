							...
							return depth({
								...
								leave: async (value: BFSValue | false) => {
									...
									for (const { arc } of adjacentTo) {
										// eslint-disable-next-line @typescript-eslint/no-non-null-assertion -- `getAdjacencyList` would fail before
										inputsLocal.set(arc.__to, outputValues.get(arc.__from)!);
									}
