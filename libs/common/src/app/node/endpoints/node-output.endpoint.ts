...
import { NodeOutputCreateDto, NodeOutputDto, NodeOutputUpdateDto } from "../dtos/output";

/**
 * Endpoint path parts for [nodes]{@link NodeOutputDto} (without global prefix).
 * The parameter goes in the middle starting with a `/`.
 */
export const NODE_OUTPUTS_ENDPOINT_PARTS = [NODES_ENDPOINT_PREFIX, "/outputs"] as const;
...
export type NodeOutput = Node["outputs"][number];
export type NodeOutputEndpoint<T extends DtoToEntity<NodeOutputDto> | NodeOutput = NodeOutput> =
